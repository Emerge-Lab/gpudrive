    ...
    def get_log_playback_actions(self, clip_actions=True):
        ...
        if self.config.dynamics_model == "delta_local":
            ...
            inferred_actions = log_trajectory[
                ...
                :,
                -6
                * constants.LOG_TRAJECTORY_LEN : -3
                * constants.LOG_TRAJECTORY_LEN,
            ].view(
                self.num_worlds,
                self.max_agent_count,
            ...
            # Extract the positions, velocities, and yaw
            position_xy = log_trajectory[
                :, :, : 2 * constants.LOG_TRAJECTORY_LEN
            ].view(
                self.num_worlds,
            ...
            velocity_xy = log_trajectory[
                :,
                :,
                2
                * constants.LOG_TRAJECTORY_LEN : 4
                * constants.LOG_TRAJECTORY_LEN,
            ].view(
                self.num_worlds,
                self.max_num_agents_in_scene,
            ...
            yaw = log_trajectory[
                ...
                :,
                4
                * constants.LOG_TRAJECTORY_LEN : 5
                * constants.LOG_TRAJECTORY_LEN,
            ].view(
                self.num_worlds,
                self.max_num_agents_in_scene,
            ...
            # Get state dynamics trajectory: (x, y, yaw, velocity x, velocity y)
            inferred_actions = torch.cat(
                (position_xy, yaw, velocity_xy), dim=-1
            )
        else:
            raise ValueError(
                f"Invalid dynamics model: {self.config.dynamics_model}"
