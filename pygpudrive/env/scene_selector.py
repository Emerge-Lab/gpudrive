...
def select_scenes(config):
    ...
    all_scenes = [scene for scene in all_scenes if scene.startswith("tfrecord")]
    ...
    if not any(scene.startswith("tfrecord") for scene in selected_scenes) or len(selected_scenes) == 0:
        raise ValueError(
