...
from algorithms.rl.sb3.ppo.reg_ippo import RegularizedIPPO
from algorithms.rl.sb3.callbacks import MultiAgentCallback
from baselines.reg_ippo.config import ExperimentConfig
...
def train(exp_config: ExperimentConfig):
    ...
    if exp_config.reg_weight > 0.0:
        saved_variables = torch.load(
            exp_config.path_to_human_reg_policy, map_location=exp_config.device
        )
        human_policy = ActorCriticPolicy(**saved_variables["data"])
        human_policy.load_state_dict(saved_variables["state_dict"])
        human_policy.to(exp_config.device)

    # INITIALIZE IPPO
    model = RegularizedIPPO(
