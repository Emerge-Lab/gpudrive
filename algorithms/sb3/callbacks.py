...
class MultiAgentCallback(BaseCallback):
    ...
    def __init__(self, config, wandb_run=None, **kwargs) -> None:
        ...
        self.perc_goal_achieved = deque(maxlen=config.logging_collection_window)
        self.perc_off_road = deque(maxlen=config.logging_collection_window)
        self.perc_veh_collisions = deque(maxlen=config.logging_collection_window)
        self.perc_non_veh_collision = deque(maxlen=config.logging_collection_window)
        self.num_agent_rollouts = deque(maxlen=config.logging_collection_window)
    ...
    def _log_metrics(self):
        ...
        metrics = {
            ...
            "metrics/perc_off_road": sum(self.perc_off_road) / total_agents * 100,
            "metrics/perc_veh_collisions": sum(self.perc_veh_collisions) / total_agents * 100,
            "metrics/perc_non_veh_collision": sum(self.perc_non_veh_collision) / total_agents * 100,
            "metrics/perc_goal_achieved": sum(self.perc_goal_achieved) / total_agents * 100,
            "metrics/perc_truncated": sum(self.perc_truncated) / total_agents * 100,
        }
        wandb.log(metrics)
    ...
    def _log_time_to_solve(self):
        ...
        if (sum(self.perc_goal_achieved) / sum(self.num_agent_rollouts) >= 0.95 and 
            self.log_first_to_95):
            wandb.log({
    ...
    def _on_rollout_end(self) -> None:
        ...
        rewards = torch.nan_to_num(self.locals["rollout_buffer"].rewards, nan=0).sum().item()
        completions = torch.nan_to_num(self.locals["rollout_buffer"].episode_starts).sum()

        if self.config.render and self.num_rollouts % self.config.render_freq == 0:
            self._create_and_log_video()

        if self.config.save_policy and self.num_rollouts % self.config.save_policy_freq == 0:
            self._save_policy_checkpoint()
    ...
    def _create_and_log_video(self, render_world_idx=0, video_title="Global step", caption="Env"):
        """Make a video and log to wandb."""
        policy = self.model
        base_env = self.locals["env"]._env
        action_tensor = torch.zeros((base_env.num_worlds, base_env.max_agent_count))

        obs = base_env.reset()
        control_mask = self.locals["env"].controlled_agent_mask.clone()
        obs = obs[control_mask].reshape(self.locals["env"].num_envs, self.locals["env"].obs_dim)
        ...
        for step_num in range(EPISODE_LENGTH):
            ...
            if step_num % 2 == 0:
                frames.append(base_env.render(world_render_idx=render_world_idx))
        ...
        wandb.log({
            f"{video_title}: {self.num_timesteps:,}": wandb.Video(
                np.moveaxis(frames, -1, 1), fps=15, format="gif", caption=caption
            )
        })

    def _save_policy_checkpoint(self) -> None:
        """Save the policy locally and to wandb."""
        path = os.path.join(self.policy_base_path, f"policy_{self.num_timesteps}.zip")
