...
def collect_rollout_for_agent_type(env, agent, agent_type_name, agent_weights, device, num_rollouts=5):
    """
    ...
    for i in tqdm(range(num_rollouts), desc=f"Agent: {agent_type_name}", unit="rollout"):
...
def collect_data_for_agent_types(env, agent, agent_configs, device, num_rollouts_per_agent=5):
    """
...
def create_dataframe_from_rollouts(rollout_data):
    ...
    for data in rollout_data:
        ...
        actions = data['actions']           # Shape: [num_envs, max_agents, episode_len, action_dim]
        ...
        for env_idx in range(num_envs):
            for agent_idx in range(max_agents):
                for time_step in range(episode_len):
                    ...
                    if torch.all(action_vec == 0) and entropy[env_idx, agent_idx, time_step] == 0:
                        continue
                    ...
                    row = {
                        ...
                        'timestep': time_step,
                        'entropy': entropy[env_idx, agent_idx, time_step].item(),
                        'acceleration': action_vec[0].item(),
                        'steering': action_vec[1].item(),
                        'logprob': logprob[env_idx, agent_idx, time_step].item(),
...
def compare_agent_types(df):
    ...
    accel_dist_by_agent = df.groupby('agent_type')['acceleration'].value_counts(normalize=True).unstack().fillna(0)
    results['acceleration_distribution'] = accel_dist_by_agent
    
    steer_dist_by_agent = df.groupby('agent_type')['steering'].value_counts(normalize=True).unstack().fillna(0)
    results['steering_distribution'] = steer_dist_by_agent
    
    # 2. Compare entropy (uncertainty) by agent type
    entropy_by_agent = df.groupby('agent_type')['entropy'].agg(['mean', 'std', 'min', 'max'])
    ...
    df['action_combo'] = df['acceleration'].astype(str) + "_" + df['steering'].astype(str)
    df_grouped = df.groupby(['agent_type', 'scenario', 'timestep'])['action_combo'].nunique().reset_index()
    ...
    entropy_range = behavior_diversity['mean_entropy'].max() - behavior_diversity['mean_entropy'].min()
    action_range = behavior_diversity['action_diversity'].max() - behavior_diversity['action_diversity'].min()
    
    if entropy_range > 1e-10:
        behavior_diversity['norm_entropy'] = (behavior_diversity['mean_entropy'] - behavior_diversity['mean_entropy'].min()) / entropy_range
    else:
        behavior_diversity['norm_entropy'] = 0.5  # Constant value if all equal
        
    if action_range > 1e-10:
        behavior_diversity['norm_action_div'] = (behavior_diversity['action_diversity'] - behavior_diversity['action_diversity'].min()) / action_range
    else:
        behavior_diversity['norm_action_div'] = 0.5  # Constant value if all equal
    
    behavior_diversity['overall_diversity'] = (behavior_diversity['norm_entropy'] + behavior_diversity['norm_action_div']) / 2
    ...
    time_series = df.groupby(['agent_type', 'timestep'])[['steering', 'acceleration']].mean()
    results['time_series'] = time_series
    
    # Print summary of comparison
    print("\n=== Agent Type Comparison ===")
    print("\nBehavioral Diversity (higher = more diverse):")
    print(behavior_diversity[['mean_entropy', 'action_diversity', 'overall_diversity']].sort_values('overall_diversity', ascending=False))
    ...
    print(df.groupby('agent_type')['acceleration'].mean().sort_values(ascending=False))
    
    print("\nAverage Steering by Agent Type (absolute value - measures turning intensity):")
    print(df.groupby('agent_type').apply(lambda x: abs(x['steering']).mean()).sort_values(ascending=False))
    
    # Analyze variation across scenarios
    scenario_variation = df.groupby(['agent_type', 'scenario'])[['acceleration', 'steering']].mean().groupby('agent_type').std()
