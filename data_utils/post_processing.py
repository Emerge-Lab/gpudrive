...
def is_valid_json_structure(file_path):
    ...
    try:
        ...
        if not isinstance(data["objects"], list) or not all(isinstance(obj, dict) and "position" in obj and "type" in obj for obj in data["objects"]):
            return False
        
        # Check if "roads" is a list of dictionaries with required "geometry"
        if not isinstance(data["roads"], list) or not all(isinstance(road, dict) and "geometry" in road for road in data["roads"]):
            return False
...
if __name__ == "__main__":
    ...
    json_files = [scene for scene in os.listdir(data_dir) if scene.endswith(".json")]

    # Initialize tqdm progress bar
    with tqdm(total=len(json_files), desc="Processing scenes", colour="green") as pbar:
        ...
        for scene in json_files:
            ...
            pbar.update(1)  # Update progress bar by 1 after processing each file
