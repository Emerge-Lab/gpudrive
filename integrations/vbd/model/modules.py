...
class Denoiser(nn.Module):
    def __init__(self, future_len=80, action_len=5, agents_len=32, steps=100):
        ...
        self.decoder = TransformerDecoder(future_len, agents_len, self._action_len)
    ...
    def forward(self, encoder_inputs, noisy_actions, diffusion_step):
        ...
        noisy_states_local = roll_out(current_states, noisy_actions,
                                action_len=self._action_len, global_frame=False)   
        
        denoised_actions_normalized = self.decoder(
            noisy_states_local, noise_level, 
...
class TransformerDecoder(nn.Module):
    def __init__(self, future_len, agents_len, action_len):
        ...
        self.attention_layers = nn.ModuleList([CrossTransformer() for _ in range(4)])
        self.encoder = nn.Sequential(nn.Linear(5, 128), nn.ReLU(), nn.Linear(128, 256))
        self.decoder = nn.Sequential(nn.Linear(256, 128), nn.ELU(), nn.Dropout(0.1), nn.Linear(128, 2))
    ...
    def generate_casual_mask(self):
        ...
        mask = torch.zeros(self._agents_len, self._seq_len, self._agents_len * self._seq_len)
            ...
            for j in range(self._agents_len):
                if i != j:
                    for t in range(self._seq_len):
                        mask[i, t, j*self._seq_len:j*self._seq_len+t+1] = 1.0
    ...
    def forward(self, noisy_trajectories, noise_level, encodings, relations, mask):
        ...
        noisy_trajectories = torch.reshape(noisy_trajectories, (-1, self._agents_len, 
                                                                self._seq_len, self._action_len, 5)) 
        ...
        query_content_list = []
        for i in range(self._agents_len):
            query_content = self.attention_layers[0](query[:, i], 
                                                     query.reshape(-1, self._agents_len*self._seq_len, 256), 
                                                     relations[:, i, :self._agents_len].repeat_interleave(self._seq_len, dim=1),
        ...
        query_content_list = []
        for i in range(self._agents_len):
            query_content = self.attention_layers[2](query_content_stack[:, i],
                                                     query_content_stack.reshape(-1, self._agents_len*self._seq_len, 256),
                                                     relations[:, i, :self._agents_len].repeat_interleave(self._seq_len, dim=1),
                                                     attn_mask=self.casual_mask[i]) # [B, T, 256]
