...
def plot_scenario(agents, futures, polyilnes, traffic_lights):
    ...
    for i in range(traffic_lights.shape[0]):
        if traffic_lights[i, 0] != 0:
            circle = plt.Circle(
                (traffic_lights[i, 0], traffic_lights[i, 1]),
                1,
                color="k",
                alpha=0.9,
                zorder=3,
...
def data_process_agent(
    ...
    agents_valid = np.pad(
        agents_valid,
        ((0, max_num_objects - agents_valid.shape[0]), (0, 0)),
        mode="constant",
    ...
    agents_traj = np.take(
        agents_traj, agent_ids, axis=0
    )  # [max_object, N step, 8]
    agents_traj = np.pad(
        agents_traj,
        ((0, max_num_objects - agents_traj.shape[0]), (0, 0), (0, 0)),
        mode="constant",
        constant_values=0.0,
    )

    # Filter out invalid agents
    agents_traj = np.where(agents_valid[..., None], agents_traj, 0.0)
    ...
    agents_history_valid = agents_valid[
        :, current_index - history_length + 1 : current_index + 1
    ]
    agents_history = agents_traj[
        :, current_index - history_length + 1 : current_index + 1
    ]  # [max_object, history_length, 8]

    future_length = agents_traj.shape[1] - history_length + 1
    agents_future = agents_traj[
        :, current_index:
    ]  # [max_object, future_length, 8]
    agents_future_valid = agents_valid[
        :, current_index:
    ]  # [max_object, future_length]

    agents_future = np.pad(
        agents_future,
        ((0, 0), (0, future_length - agents_future.shape[1]), (0, 0)),
        mode="constant",
        constant_values=0.0,
    )
    agents_future_valid = np.pad(
        agents_future_valid,
    ...
    agents_type = np.pad(
        agents_type,
        (0, max_num_objects - agents_type.shape[0]),
        mode="constant",
        constant_values=0,
    )
    agents_type = np.where(agents_type > 0, agents_type, 0)

    is_modeled = np.take(np.asarray(metadata.is_modeled), agent_ids, axis=0)
    is_modeled = np.pad(
        is_modeled,
        (0, max_num_objects - is_modeled.shape[0]),
        mode="constant",
        constant_values=False,
    )
    objects_of_interest = np.take(
        np.asarray(metadata.objects_of_interest), agent_ids, axis=0
    )
    objects_of_interest = np.pad(
        objects_of_interest,
        (0, max_num_objects - objects_of_interest.shape[0]),
        mode="constant",
        constant_values=False,
    )
    ...
    agents_interested = np.where(
        (is_modeled | objects_of_interest), 10, agents_interested
    )

    # Padding agent_ids
    agent_ids = np.pad(
        agent_ids,
        (0, max_num_objects - len(agent_ids)),
        mode="constant",
        constant_values=-1,
    )
...
def data_process_traffic_light(
    scenario: datatypes.SimulatorState, current_index: int = 10
):
    """
...
def data_process_scenario(
    ...
    current_positions = np.asarray(scenario.log_trajectory.xy)[
        :, current_index
    ]
    ...
    relations = calculate_relations(
        agents_history, polylines, traffic_light_points
    )
