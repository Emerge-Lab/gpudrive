...
def create_iter(
...
) -> iter(Tuple[str, datatypes.SimulatorState]):
    ...
    return decode_bytes(
        dataloader.get_data_generator(data_config, _preprocess, _postprocess)
    )
...
def smooth_scenario(
    ...
    for i in range(num_agent):
        ...
        std = np.clip(
            np.std(vel_valid, axis=-2, keepdims=True), a_min=0.1, a_max=None
        )
        ...
        smoothed_vel_x[i, first_valid_idx : last_valid_idx + 1] = vel_smoothed[
            :, 0
        ]
        smoothed_vel_y[i, first_valid_idx : last_valid_idx + 1] = vel_smoothed[
            :, 1
        ]
        smoothed_yaw[i, first_valid_idx : last_valid_idx + 1] = np.arctan2(
            vel_smoothed[:, 2], vel_smoothed[:, 3]
        )
    ...
    scenario.log_trajectory.valid = np.logical_and(
        original_valid, smoothed_valid
    )
