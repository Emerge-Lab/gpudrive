...
def sim_init():
    ...
    params.maxNumControlledVehicles = (
...
def test_forward_inverse_delta_dynamics(sim_init):
    ...
    assert torch.allclose(
        position, pos[0], atol=1e-5
    ), f"Position mismatch: {position} vs {pos[0]}"
    assert (
        pytest.approx(heading.item(), abs=1e-5) == headings[0].item()
    ), f"Heading mismatch: {heading.item()} vs {headings[0].item()}"
    assert (
        pytest.approx(speed.item(), abs=1e-5) == torch.norm(vel[0]).item()
    ), f"Speed mismatch: {speed.item()} vs {torch.norm(vel[0]).item()}"
    ...
    assert torch.allclose(
        position, pos[1], atol=1e-5
    ), f"Position mismatch: {position} vs {pos[1]} at step {1}"
    assert (
        pytest.approx(heading.item(), abs=1e-5) == headings[1].item()
    ), f"Heading mismatch: {heading.item()} vs {headings[1].item()}"
    assert (
        pytest.approx(speed.item(), abs=1e-5) == torch.norm(vel[1]).item()
    ), f"Speed mismatch: {speed.item()} vs {torch.norm(vel[1]).item()}"
