...
def compute_gae(torch.Tensor dones, torch.Tensor values,
                torch.Tensor rewards, float gamma, float gae_lambda):
    '''Fast implementation of Generalized Advantage Estimation (GAE) with PyTorch'''
    device = rewards.device
    cdef int num_steps = rewards.size(0)
    advantages = torch.zeros(num_steps, dtype=torch.float32, device=device)
    cdef float lastgaelam = 0.0
    cdef float nextnonterminal, delta
    cdef int t
    for t in reversed(range(num_steps - 1)):
        nextnonterminal = 1.0 - dones[t + 1]
        delta = rewards[t + 1] + gamma * values[t + 1] * nextnonterminal - values[t]
        lastgaelam = delta + gamma * gae_lambda * nextnonterminal * lastgaelam
