...
def rollout(envs, agent):
    next_obs, rews, terminations, truncateds, infos, env_ids, mask = envs.reset(seed=args.seed)
    next_obs = torch.Tensor(next_obs).to(device)
    indices = get_indices_from_mask(mask)
    next_obs = next_obs[indices]
    terminations = terminations[indices]
    rews = rews[indices]
    while True:
        if torch.all(terminations):
            print("Rollout reward: ", torch.sum(rews).item())
            break;
        with torch.no_grad():
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        print(f"{global_step=}, {values.mean()=}, {values.std()=}, {rewards.mean()=}, {rewards.std()=}")
