...
void createPersistentEntities(Engine &ctx, Map *map) {
    ...
    for (CountT agentCtr = 0; agentCtr < map->numObjects; ++agentCtr) {
        if(agentIdx >= consts::maxNumObjects)
            break;
    ...
    {
        if(roadIdx >= consts::maxNumRgPoints)
            break;
