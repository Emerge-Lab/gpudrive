...
class NeuralNetWithHistory(
    ...
    def __init__(
        ...
        log_history =5,
        closest_k_partners_in_history ='all',
        ...
        self.num_modes = 4  # Ego, partner, road graph, history
        self.dropout = dropout
        self.act_func = nn.Tanh() if act_func == "tanh" else nn.GELU()
        self.log_history_step = log_history
        ...
        self.road_graph_idx = 2984  # change hardcoded value

        if config is not None:
            self.config = config
            if 'reward_type' in self.config and isinstance(self.config,Box):
    ...
    def encode_observations(self, observation):
        ego_state, road_objects, road_graph, history = self.unpack_obs(observation)
        ...
        history_embed, _ = self.history_embed(history).max(dim=1)

        embed = torch.cat([ego_embed, partner_embed, road_map_embed, history_embed], dim=1)
    ...
    def unpack_obs(self, obs_flat):
        ...
        """
        #print(f"observations are {obs_flat.shape}, in network")
        # Unpack modalities
        ego_state = obs_flat[:, : self.ego_state_idx]
        partner_obs = obs_flat[:, self.ego_state_idx : self.partner_obs_idx]
        roadgraph_obs = obs_flat[:, self.partner_obs_idx : self.road_graph_idx]
        history_obs = obs_flat[:,self.road_graph_idx: ]
        ...
        if self.closest_k_partners_in_history == "all":
            history = history_obs.view(
                -1, self.k_trials *(int(self.num_steps/self.log_history_step)+1) * self.max_observable_agents , constants.PARTNER_FEAT_DIM
            )
        else:
            history = history_obs.view(
                -1, self.k_trials *(int(self.num_steps/self.log_history_step)+1)  * self.closest_k_partners_in_history , constants.PARTNER_FEAT_DIM
            )

        return ego_state, road_objects, road_graph, history
